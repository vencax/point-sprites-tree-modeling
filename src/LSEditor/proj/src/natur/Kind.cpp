...
CKind::CKind( LPDIRECT3DDEVICE9 d, LPDIRECT3DVERTEXBUFFER9	suppVB, string curDir) : 
m_dev( d),
m_SuppVertBuf( suppVB)
{
	...
	// init randSeed for tree
	srand( (unsigned)time( NULL ) );

	// init default filenames for textures

	// prepare default path
	defaultResPath = curDir;
	defaultResPath += "\\res\\textures\\default\\";

	string pom;

	char buffer[ 32];

	// prepare default filenames
	for( int i = 0; i < NUMBER_OF_PS_TEXTURES; i++)
	{
		sprintf( buffer, "%i.png", i);

		pom = defaultResPath + buffer;

		m_pTextureFilenames[ i] = pom;	
...
{		
	...
	m_dev->SetStreamSource( 0, GetTree( )->GetPSVertexBuf( ), 0, sizeof( D3DXVECTOR3) );
	...
	for( int i = 0; i < NUMBER_OF_PS_TEXTURES; i++)
	{
		...
		{
			curr = GetTree( )->GetLevelHeads( j)->GetChainPointers( )->NextInPSDraw[ i];
			while( curr)
			{
				// set world matrix
...
CKind::DrawSiplifiedBranches( CNO *tree)
{
	...
	CBranch *curr;
	curr = GetTree( )->GetLevelHeads( level)->GetChainPointers( )->NextSimplified;
	...
	if( level < NUM_LVLS-1)
	{
		curr = GetTree( )->GetLevelHeads( level+1)->GetChainPointers( )->NextSimplified;
...
CKind::DrawCrown( CNO* tree)
{
	...
	CBranch *curr;
	curr = GetTree( )->GetLevelHeads( 0)->GetChainPointers( )->NextOnLevel;
...
CKind::GenerateTextures( void)
{
	...
	for( int i = 0; i < NUMBER_OF_PS_TEXTURES; i++)
	{
		...
		if( FAILED( D3DXCreateTextureFromFileExA( m_dev, m_pTextureFilenames[ i].c_str( ), D3DX_DEFAULT,D3DX_DEFAULT,D3DX_DEFAULT,NULL,D3DFMT_UNKNOWN,D3DPOOL_MANAGED,D3DX_DEFAULT,D3DX_DEFAULT,0xffffffff,NULL,NULL,&m_textures[ 3][ i]) ) )
		{
	...
	char *pom = new char[ defaultResPath.size( ) + 20];
	...
	for( int i = 0; i < NUM_OF_SIMPLFD_TEXTRS0; i++)
	{
		...
		sprintf( pom, "%slvl0Tex%d.png", &defaultResPath[ 0], i);

		if( FAILED( D3DXCreateTextureFromFileExA( m_dev, pom, D3DX_DEFAULT,D3DX_DEFAULT,D3DX_DEFAULT,NULL,D3DFMT_UNKNOWN,D3DPOOL_MANAGED,D3DX_DEFAULT,D3DX_DEFAULT,0xffffffff,NULL,NULL,&m_textures[ 0][ i]) ) )
		{
			MessageBox( 0, "Creation of texture failed!", "Error", MB_OK | MB_ICONEXCLAMATION);
			return false;
		}			
	...
	for( int i = 0; i < NUM_OF_SIMPLFD_TEXTRS1; i++)
	{
		...
		sprintf( pom, "%slvl1Tex%d.png", &defaultResPath[ 0], i);

		if( FAILED( D3DXCreateTextureFromFileExA( m_dev, pom, D3DX_DEFAULT,D3DX_DEFAULT,D3DX_DEFAULT,NULL,D3DFMT_UNKNOWN,D3DPOOL_MANAGED,D3DX_DEFAULT,D3DX_DEFAULT,0xffffffff,NULL,NULL,&m_textures[ 1][ i]) ) )
		{
			MessageBox( 0, "Creation of texture failed!", "Error", MB_OK | MB_ICONEXCLAMATION);
			return false;
		}			
	...
	for( int i = 0; i < NUM_OF_SIMPLFD_TEXTRS2; i++)
	{
		...
		sprintf( pom, "%slvl2Tex%d.png", &defaultResPath[ 0], i);

		if( FAILED( D3DXCreateTextureFromFileExA( m_dev, pom, D3DX_DEFAULT,D3DX_DEFAULT,D3DX_DEFAULT,NULL,D3DFMT_UNKNOWN,D3DPOOL_MANAGED,D3DX_DEFAULT,D3DX_DEFAULT,0xffffffff,NULL,NULL,&m_textures[ 2][ i]) ) )
		{
			MessageBox( 0, "Creation of texture failed!", "Error", MB_OK | MB_ICONEXCLAMATION);
			return false;
		}			
	...
	// crust texture
	sprintf( pom, "%scrust.png", &defaultResPath[ 0]);
