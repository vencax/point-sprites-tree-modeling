...
////////////////////////////////////////////
// only for using in MFC
////////////////////////////////////////////
bool
	LSystem::LoadRulesFromText( char *buf)
{
	char left;
	short prob;
	string right;
...
string 
	LSystem::getLineFromText( char *f, unsigned long *pos)
// reads one single line from char field and return it
/// reads only the segment before '#'. Rest of line is omited. 
// Return first valid line( first no-comment line)
// Line does not contain comments introduced by '#'
{
	...
	// try to get line from file
	while( ( f[ *pos] != '\0') && ( s.size( ) == 0) )
	{
		while( ( f[ *pos] != '\0') && ( f[ *pos] != '#') && ( f[ *pos] != '\r') && ( f[ *pos] != '\n') )
		{
			s += f[ *pos];
			(*pos)++;
		}
		switch( f[ *pos]){
		case '#':							// move on the next line
			while( f[ *pos] != '\r')
				(*pos)++;
			(*pos) += 2;	// skip on the next line		
			break;
...
/* returns a Rules String for the Win32 dialog box */
string LSystem::getRulesString( void)
{
	string value = "";
	char probab[ 4];
	variationType* variations;

	// go through all the rules and print them
	for( rulesType::iterator itr = m_rules.begin() ; itr != m_rules.end() ; itr++)
	{
		if( itr->first == 'X')	// skip axiom
			continue;
		variations = itr->second;
		for( variationType::iterator i = variations->begin( ); i != variations->end( ); i++)
		{
			sprintf( probab, "%i", i->first);
			value += itr->first;
			value += "(";
			value += probab;
			value += ")=";
